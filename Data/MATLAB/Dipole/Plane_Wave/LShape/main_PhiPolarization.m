...
%%
[E_theta,~,~,phi]=FarField_PhiCut(0,Data,I);
...
%%
[~,E_phi,~,theta]=FarField_ThetaCut(60,Data,I);
E_phi    	=   E_phi./max(abs(E_phi));
figure()
PolardB(theta,20*log10(abs(E_phi)),[-25 0],6,'-k')
title('$20\log_{10}|E_{\theta}|$ [dB]','Interpret','Latex')
%%
...
ylabel('$\sigma_{\theta\varphi}/\lambda^2 $ [dB]','Interpret','Latex')
set(gca,'TickLabel','Latex')
xlim([-180 180])
% ylim([-30 10])
%%
...
xlabel('$\theta$ [deg]','Interpret','Latex')
ylabel('$\sigma_{\varphi\varphi}/\lambda^2 $ [dB]','Interpret','Latex')
